...
-- Check Venue Availability and Register Wedding
DECLARE
    v_available_count INTEGER;
    v_venue_id VARCHAR2(10) := 'V1';  -- Example Venue ID
    v_registration_date DATE := TO_DATE('2024-12-15', 'YYYY-MM-DD'); -- Example wedding date
    v_client_id VARCHAR2(10) := 'P1';  -- Example Client ID for registration
BEGIN
    -- Check venue availability
    SELECT COUNT(*) INTO v_available_count
    FROM Registration
    WHERE vid = v_venue_id AND wdate = v_registration_date;

    IF v_available_count > 0 THEN
        DBMS_OUTPUT.PUT_LINE('The venue is already booked for the selected date.');
    ELSE
        -- Register the wedding if venue is available
        INSERT INTO Registration (reg_id, wdate, vid, cid, tid, pid)
        VALUES ('R11', v_registration_date, v_venue_id, 'C1', 'T1', v_client_id);
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Registration successful for venue ' || v_venue_id);
    END IF;
END;
/

-- Trigger: Automatically Create Tasks for Registration
CREATE OR REPLACE TRIGGER create_task_for_reg
AFTER INSERT ON Registration
FOR EACH ROW
BEGIN
    -- Task for Event Planner
    INSERT INTO Task (task_id, task_name, assigned_to, deadline, status)
    VALUES ('T' || :NEW.reg_id, 'Decorate the Venue', 'E1', :NEW.wdate, 'Pending');
    
    -- Task for Caterer
    INSERT INTO Task (task_id, task_name, assigned_to, deadline, status)
    VALUES ('T' || :NEW.reg_id, 'Arrange Catering', 'E2', :NEW.wdate, 'Pending');
    
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Tasks automatically created for registration ' || :NEW.reg_id);
END;
/

-- Display Tasks Assigned to Event Planner (Employee E1)
DECLARE
    CURSOR task_cursor IS
        SELECT task_id, task_name, deadline, status
        FROM Task
        WHERE assigned_to = 'E1';
BEGIN
    FOR task IN task_cursor LOOP
        DBMS_OUTPUT.PUT_LINE('Task ID: ' || task.task_id || ', Task Name: ' || task.task_name || ', Deadline: ' || task.deadline || ', Status: ' || task.status);
    END LOOP;
END;
/

-- Error Handling Example: Duplicate Task Insertion
DECLARE
    v_task_id VARCHAR2(10) := 'T2';  
BEGIN
    BEGIN
        -- Attempt to insert duplicate task ID
        INSERT INTO Task (task_id, task_name, assigned_to, deadline, status)
        VALUES (v_task_id, 'Venue Decoration', 'E1', TO_DATE('2024-12-15', 'YYYY-MM-DD'), 'Pending');
        COMMIT;
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            DBMS_OUTPUT.PUT_LINE('Error: Task ' || v_task_id || ' already exists.');
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Unexpected error: ' || SQLERRM);
    END;
END;
/
-- 1. UNION Operation: Combine all unique guests from both tables
SELECT name, phone 
FROM Guest_Bride
UNION
SELECT name, phone 
...
-- 2. INTERSECTION Operation: Find guests who are common in both tables
SELECT name, phone 
FROM Guest_Bride
INTERSECT
SELECT name, phone 
...
-- 3. SET DIFFERENCE Operation: Find guests in Guest_Bride but not in Guest_Groom
SELECT name, phone 
FROM Guest_Bride
MINUS
SELECT name, phone 
...
-- 4. SET DIFFERENCE Operation: Find guests in Guest_Groom but not in Guest_Bride
SELECT name, phone 
FROM Guest_Groom
MINUS
SELECT name, phone 
FROM Guest_Bride;
-- Total Number of Registrations for Each Client
SELECT c.name, COUNT(r.reg_id) AS total_registrations
FROM Clients c
LEFT JOIN Registration r ON c.pid = r.pid
GROUP BY c.name;

-- Average Price of Venues
SELECT AVG(price) AS avg_venue_price
FROM Venue;

-- Total Number of Guests (Groom and Bride) for Each Registration
SELECT r.reg_id, 
       (SELECT COUNT(*) FROM Guest_Groom gg WHERE gg.groom_id = r.reg_id) + 
       (SELECT COUNT(*) FROM Guest_Bride gb WHERE gb.bride_id = r.reg_id) AS total_guests
FROM Registration r;

-- Total Cost of Catering and Venue for Each Registration
SELECT r.reg_id, 
       v.price + c.price AS total_cost
FROM Registration r
JOIN Venue v ON r.vid = v.vid
JOIN Catering c ON r.cid = c.cid;

-- Most Expensive Theme Chosen
SELECT t.name, MAX(t.price) AS max_theme_price
FROM Theme t;

-- Task Status Count for Each Employee
SELECT e.name, t.status, COUNT(t.task_id) AS task_count
FROM Employee e
LEFT JOIN Task t ON e.emp_id = t.assigned_to
GROUP BY e.name, t.status;

-- Average Price of Themes for Each Client
SELECT c.name, AVG(t.price) AS avg_theme_price
FROM Clients c
JOIN Registration r ON c.pid = r.pid
JOIN Theme t ON r.tid = t.tid
GROUP BY c.name;
